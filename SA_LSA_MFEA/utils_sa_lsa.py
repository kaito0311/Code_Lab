...
def create_child_v3(population, parent, skill_factor, rmp, S, xichma, tasks):
    ...
    if skill_factor[parent[0]] == skill_factor[parent[1]]:
        ...
        factorial_cost_child[0] = tasks[skill_factor_child[0]].calculate_fitness(
            childs[0]
        )
        factorial_cost_child[1] = tasks[skill_factor_child[1]].calculate_fitness(
            childs[1]
        )
    else:
        r = np.random.rand()
        ...
        delta = calculate_improvement_percentage(
            population, parent, childs, tasks, skill_factor, skill_factor_child
        )

        if delta > 0:
            index = index_convert_matrix_to_1D(
                skill_factor[parent[0]], skill_factor[parent[1]], len(tasks)
            )
...
def calculate_improvement_percentage(
    ...
    if skill_factor_child[1] == skill_factor[parent[1]]:
...
def Update_Success_History_Memory(M, S, xichma, number_tasks):
    ...
    for i in range(number_tasks):
        ...
        while j < number_tasks:
            ...
            if len(S[index]) != 0:
                M[index].update_M(np.sum(np.array(xichma[index]) * np.array(S[index])) / np.sum(np.array(xichma[index])))
            j += 1
    return M
...
def update(population, number_population, skill_factor, scalar_fitness, factorial_cost):
    """
    number population : an array has shape (number_tasks, ). bieu thi so luong ca the con dc giu trong quan the
    """
    delete_index = []
