...
class Model:
  ...
  def density(self, subpop):
    ...
    return prob # day xac suat cua mot ca the 
...
def learn_rmp(subpops, D):
  ...
  for k in range(K - 1):
    for j in range(k + 1, K):
      probmatrix = [np.ones([models[k].num_sample, 2]), # shape (2, 50, 2)
                    np.ones([models[j].num_sample, 2])]
      probmatrix[0][:, 0] = models[k].density(subpops[k]) # tính toán rmp của quần thể con thứ k tương ứng với phân bố của j
